...
def predictPage():
    try:
        if request.method == 'POST':
            ...
            logging.debug("Received form data: %s", to_predict_dict)
            ...
            to_predict_list = list(map(float, list(to_predict_dict.values())))
            logging.debug("Converted data to list: %s", to_predict_list)
        ...
        message = "An error occurred during prediction: {}".format(str(e))
        return render_template("home.html", message=message)

    # If no prediction was made, return to home page with a message
    message = "No prediction was made. Please enter valid data."
    return render_template("home.html", message=message)
